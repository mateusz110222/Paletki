...
import { Wrench, CheckSquare, Sparkles, User, ShieldCheck, X, FileText, CheckCircle2, AlertTriangle, AlertCircle, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300" id="maintenance-panel-container">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
        <div className="md:col-span-1 bg-brand-surface p-6 rounded-xl border-l-4 border-brand-accent border-y border-r flex flex-col justify-between">
          <span className="text-[10px] font-bold text-brand-text-muted uppercase tracking-wider">{t('maintenance_queue')}</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-4xl font-black text-brand-text">{data.repairPallets.length + data.routinePallets.length}</span>
            <span className="text-xs font-bold text-brand-text-muted">{t('pallets_count')}</span>
          </div>
          <div className="mt-4 py-1 px-2 bg-brand-accent/10 border border-brand-accent/20 rounded inline-flex items-center gap-2 w-fit">
            <div className="w-1.5 h-1.5 rounded-full bg-brand-accent animate-pulse"></div>
            <span className="text-[9px] font-bold text-brand-accent uppercase">{t('priority_high')}</span>
          </div>
        </div>

        <div className="md:col-span-1 bg-brand-surface p-6 rounded-xl border border-brand-border flex flex-col justify-between">
          <span className="text-[10px] font-bold text-brand-text-muted uppercase tracking-wider">{t('for_repair')}</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-4xl font-black text-red-500">{data.repairPallets.length}</span>
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-red-500/70 uppercase leading-tight">{t('intervention_required_line1')}</span>
              <span className="text-[10px] font-bold text-red-500/70 uppercase leading-tight">{t('intervention_required_line2')}</span>
            </div>
          </div>
          <div className="mt-4 flex items-center gap-1.5">
            <AlertTriangle size={14} className="text-red-500" />
            <span className="text-[10px] font-medium text-brand-text-muted italic">{t('reported_damage')}</span>
          </div>
        </div>

        <div className="md:col-span-1 bg-brand-surface p-6 rounded-xl border border-brand-border flex flex-col justify-between">
          <span className="text-[10px] font-bold text-brand-text-muted uppercase tracking-wider">{t('routine_inspections')}</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-4xl font-black text-yellow-500">{data.routinePallets.length}</span>
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-yellow-500/70 uppercase leading-tight">{t('awaiting_service_line1')}</span>
              <span className="text-[10px] font-bold text-yellow-500/70 uppercase leading-tight">{t('awaiting_service_line2')}</span>
            </div>
          </div>
          <div className="mt-4 flex items-center gap-1.5">
            <RefreshCw size={14} className="text-yellow-500" />
            <span className="text-[10px] font-medium text-brand-text-muted italic">{t('cycle_limit_exceeded')}</span>
          </div>
        </div>

        <div className="md:col-span-1 bg-brand-surface p-6 rounded-xl border border-brand-border flex flex-col justify-between">
          <span className="text-[10px] font-bold text-brand-text-muted uppercase tracking-wider">{t('technician_status')}</span>
          ...
          <div className="mt-4 flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-medium text-brand-text-muted italic">{t('service_permissions_ok')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-brand-surface rounded-xl border border-brand-border overflow-hidden">
        ...
        <div className="flex border-b border-brand-border bg-brand-surface-high/30">
          ...
          >
            <AlertCircle size={16} />
            {t('repairs_tab')}
            {data.repairPallets.length > 0 && (
              <span className="ml-2 px-1.5 py-0.5 bg-red-500 text-white text-[9px] rounded-full">{data.repairPallets.length}</span>
            )}
          </button>
          <button
          ...
          >
            <Wrench size={16} />
            {t('routine_tab')}
            {data.routinePallets.length > 0 && (
              <span className="ml-2 px-1.5 py-0.5 bg-yellow-500 text-brand-bg text-[9px] rounded-full">{data.routinePallets.length}</span>
            )}
          </button>
        </div>
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {(data.activeTab === 'repairs' ? data.repairPallets : data.routinePallets).map((p: any) => (
              <div
                key={p.pallet_id}
              ...
              >
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div className="px-2 py-1 bg-brand-surface-high border border-brand-border rounded font-mono text-xs font-bold text-brand-accent">
                      {p.pallet_id}
                    </div>
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-mono font-bold text-brand-text">
                        {p.current_cycles} <span className="text-brand-text-muted/60">/ {p.max_cycles}</span>
                      </p>
                    </div>
                    {p.block_reason && (
            ...
            {(data.activeTab === 'repairs' ? data.repairPallets : data.routinePallets).length === 0 && (
      ...
      {data.selectedPallet && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-brand-bg/80 backdrop-blur-md" onClick={() => actions.setSelectedPallet(null)}></div>
          <div className="relative bg-brand-surface border border-brand-border w-full max-w-xl rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)]">
            {/* Modal Header */}
            <div className="bg-brand-surface-high p-6 border-b border-brand-border flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-brand-text-muted font-mono">{t('pallet_id_label')}: <span className="text-brand-accent font-bold">{data.selectedPallet.pallet_id}</span></p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={actions.handleReturnToProduction} className="p-8 space-y-6">
              {data.modalError && (
                <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg flex items-start gap-3 animate-in shake duration-300">
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center gap-4 p-4 bg-brand-bg border border-brand-border rounded-xl cursor-pointer hover:border-brand-accent/40 transition-all group">
                  <div className="relative flex items-center">
                    ...
                    />
                    <div className="h-6 w-6 bg-brand-surface border-2 border-brand-border rounded-md transition-all flex items-center justify-center">
                      <Sparkles size={14} className={`text-brand-bg transition-transform ${data.washConfirm ? 'scale-100' : 'scale-0'}`} />
                    </div>
                  </div>
                  <div className="flex-1">
                    <span className="text-xs font-bold text-brand-text block">{t('ultrasonic_washing')}</span>
                    <span className="text-[10px] text-brand-text-muted">{t('remove_contaminants')}</span>
                  </div>
                </label>
                ...
                <label className="flex items-center gap-4 p-4 bg-brand-bg border border-brand-border rounded-xl cursor-pointer hover:border-brand-accent/40 transition-all group">
                  <div className="relative flex items-center">
                    ...
                    />
                    <div className="h-6 w-6 bg-brand-surface border-2 border-brand-border rounded-md transition-all flex items-center justify-center">
                      <Sparkles size={14} className={`text-brand-bg transition-transform ${data.fluxConfirm ? 'scale-100' : 'scale-0'}`} />
                    </div>
                  </div>
                  <div className="flex-1">
                    <span className="text-xs font-bold text-brand-text block">{t('flux_residue_inspection')}</span>
                    <span className="text-[10px] text-brand-text-muted">{t('verify_pin_cleanliness')}</span>
                  </div>
                </label>
              </div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-brand-text-muted uppercase tracking-widest">{t('detailed_work_description')}</label>
                <textarea
                  rows={3}
