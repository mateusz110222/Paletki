            ...
            <div className="dashboard-public min-h-screen grid place-items-center p-6 text-white">
                <div className="text-center">
                    <div className="mx-auto grid size-14 place-items-center rounded-2xl border border-indigo-400/25 bg-indigo-400/10">
                        <RefreshCw className="animate-spin text-indigo-300" size={24}/>
                    </div>
    ...
    return (
        <div className="dashboard-public min-h-screen text-slate-100 selection:bg-indigo-400 selection:text-slate-950">
            <header className="sticky top-0 z-30 border-b border-white/[0.07] bg-brand-bg/90 backdrop-blur-xl">
                <div className="mx-auto flex max-w-[1800px] flex-wrap items-center justify-between gap-4 px-5 py-4 sm:px-7 lg:px-10">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                <span className="rounded border border-emerald-400/20 bg-emerald-400/10 px-1.5 py-0.5 text-[8px] font-black tracking-[0.15em] text-emerald-300">LIVE</span>
                            </div>
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2 sm:gap-3">
                        ...
                        <div className="rounded-lg border border-white/[0.07] bg-white/2.5 px-3 py-2 text-right">
                            <p className="text-[10px] font-bold text-slate-200">{clock}</p>
                            <p className="mt-0.5 flex items-center justify-end gap-1 text-[9px] text-slate-500">
            ...
            <main className="mx-auto max-w-[1800px] space-y-5 px-5 py-6 sm:px-7 lg:px-10 lg:py-8">
                <section className="grid grid-cols-2 gap-3 lg:grid-cols-5 lg:gap-4">
                    ...
                    <KpiCard label={labels.washSoon} value={metrics.dueSoon.length} detail="≥ 80%" icon={<TimerReset size={20}/>} tone="via-amber-300/70"/>
                    <KpiCard label={labels.inWash} value={metrics.awaitingWash.length} detail={`${statusCounts.Damaged} ${labels.damaged.toLowerCase()}`} icon={<Droplets size={20}/>} tone="via-cyan-300/70"/>
                    <div className="col-span-2 lg:col-span-1">
                        <KpiCard label={labels.avgService} value={formatDuration(service.average_minutes_30d, language)} detail={`${service.completed_30d} · ${labels.last30Days}`} icon={<Clock3 size={20}/>} tone="via-fuchsia-400/70"/>
                    </div>
                </section>
                ...
                <section className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.65fr)_minmax(360px,0.85fr)]">
                    <div className="dashboard-panel overflow-hidden rounded-2xl">
                        <PanelHeader icon={<Droplets size={17}/>} title={labels.upcomingTitle} subtitle={labels.upcomingSubtitle} aside={
                            <span className="rounded-lg border border-amber-300/20 bg-amber-300/10 px-2.5 py-1 text-[10px] font-black text-amber-200">{metrics.upcoming.length}</span>
                        }/>
                        <div className="overflow-x-auto">
                            {metrics.upcoming.length > 0 ? (
                                <table className="w-full min-w-170 border-collapse text-left">
                                    <thead>
                                    <tr className="border-b border-white/5.5 text-[9px] font-bold uppercase tracking-[0.14em] text-slate-500">
                                        ...
                                        return (
                                            <tr key={pallet.pallet_id} className="border-b border-white/4.5 last:border-0 hover:bg-white/2.5">
                                                ...
                                                <td className="px-4 py-3.5">
                                                    ...
                                                    <p className="mt-0.5 max-w-52 truncate text-[10px] text-slate-500">{pallet.model}</p>
                                                </td>
                                                <td className="px-4 py-3.5">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="w-9 font-mono text-[10px] font-bold text-slate-300">{progress}%</span>
                                                    </div>
                                                </td>
                                                <td className="px-4 py-3.5 font-mono text-[10px] font-semibold text-slate-300">
                                ...
                                <EmptyState icon={<CheckCircle2 size={24}/>} text={labels.noUpcoming}/>
                            )}
                        </div>
                    </div>

                    <div className="dashboard-panel overflow-hidden rounded-2xl">
                        <PanelHeader icon={<BarChart3 size={17}/>} title={labels.serviceTrend} subtitle={labels.serviceTrendSubtitle}/>
                        <div className="p-5 sm:p-6">
                            <div className="mb-5 flex items-end justify-between gap-4">
                                <div>
                                    <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-slate-500">{labels.average}</p>
                                    <p className="mt-1 text-2xl font-black text-white">{formatDuration(service.average_minutes_30d, language)}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-slate-500">{labels.completed}</p>
                                        ...
                                        return (
                                            <div key={point.day} className="group flex h-full min-w-0 flex-1 flex-col justify-end">
                                                <div className="relative flex flex-1 items-end justify-center">
                                                    <div className="absolute bottom-full left-1/2 z-10 mb-2 hidden -translate-x-1/2 whitespace-nowrap rounded-lg border border-white/10 bg-slate-950 px-2.5 py-2 text-[9px] shadow-xl group-hover:block">
                                                ...
                                                <span className="mt-2 h-4 truncate text-center text-[8px] font-semibold text-slate-500">
                ...
                <section className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
                    <div className="dashboard-panel overflow-hidden rounded-2xl">
                        <PanelHeader icon={<Wrench size={17}/>} title={labels.queue} subtitle={labels.queueSubtitle}/>
                            ...
                            <div className="grid gap-px bg-white/5.5 sm:grid-cols-2 xl:grid-cols-3">
                                {metrics.serviceQueue.slice(0, 9).map((pallet) => (
                                    <div key={pallet.pallet_id} className="bg-[#101622] p-4 hover:bg-[#131b29]">
                                        ...
                                        <p className="mt-3 truncate text-[11px] font-semibold text-slate-300">{pallet.project} · {pallet.model}</p>
                                        <div className="mt-2 flex items-center gap-1.5 text-[10px] text-slate-500">
                                            <Clock3 size={12}/>{labels.waiting}: <span className="font-bold text-slate-300">{formatDuration(elapsedMinutes(pallet.status_changed_at, effectiveNow), language) || labels.justNow}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <EmptyState icon={<CheckCircle2 size={24}/>} text={labels.queueEmpty}/>
                        )}
                    </div>

                    <div className="dashboard-panel overflow-hidden rounded-2xl">
                        <PanelHeader icon={<Gauge size={17}/>} title={labels.projectLoad} subtitle={labels.projectLoadSubtitle}/>
                                ...
                                return (
                                    <div key={project.name}>
                                        <div className="mb-2 flex items-center justify-between gap-3">
                                            ...
                                            <span className="shrink-0 font-mono text-[10px] text-slate-400"><strong className={project.attention ? 'text-amber-200' : 'text-emerald-300'}>{project.attention}</strong> / {project.total}</span>
                                        </div>
