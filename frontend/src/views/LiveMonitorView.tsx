...
export const LiveMonitorView: React.FC = () => {
        ...
        return (
            <div className="dashboard-public grid min-h-screen place-items-center p-6 text-white">
                <div className="text-center">
                    <RefreshCw className="mx-auto animate-spin text-indigo-300" size="1.75rem"/>
    ...
    const getStatusTheme = (percentage: number, total: number) => {
        if (total === 0) {
            return {
                ...
                icon: <Package size="1rem" className="text-brand-text-muted"/>,
        ...
        if (percentage >= 80) {
            return {
                ...
                icon: <CheckCircle2 size="1rem" className="text-emerald-400"/>,
        ...
        if (percentage >= 40) {
            return {
                ...
                icon: <AlertTriangle size="1rem" className="text-amber-400"/>,
        ...
        return {
            ...
            icon: <AlertCircle size="1rem" className="text-rose-400"/>,
    ...
    return (
        <div className={`dashboard-screen ${stationRowMode ? 'flex min-h-screen flex-col lg:h-dvh lg:overflow-auto' : 'min-h-screen'} bg-brand-bg text-brand-text`}>
            <DisplayModeControl/>
            <header className="shrink-0 border-b border-brand-border bg-brand-surface/95 backdrop-blur">
                <div className={`mx-auto flex w-full flex-wrap items-center justify-between gap-4 px-5 sm:px-8 ${stationRowMode ? 'py-[clamp(0.55rem,1.35vh,1rem)]' : 'py-4'}`}>
                    <div className="flex items-center gap-3">
                        <div className="grid size-10 place-items-center rounded-xl border border-brand-accent/30 bg-brand-accent/10 text-brand-accent"><Tv size="1.25rem"/></div>
                        <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        <div className="hidden rounded-lg border border-brand-accent/20 bg-brand-accent/8 px-3 py-2 text-right sm:block">
                            <p className="text-[0.5625rem] font-bold uppercase tracking-wider text-brand-accent">
                                {showAll ? 'ALL' : `${t('dashboard_station')}: ${selectedStation?.station}`}
                            </p>
                            <p className="mt-0.5 max-w-44 truncate text-[0.625rem] font-semibold text-brand-text">
                                {showAll
                        ...
                        <button type="button" onClick={() => setSearchParams({})} className="min-h-10 rounded-lg border border-brand-border bg-brand-bg px-3 text-[0.625rem] font-bold text-brand-text-muted hover:border-brand-accent/50 hover:text-brand-text">
                            {t('dashboard_change_station')}
                        </button>
                        <span className="hidden items-center gap-1.5 text-[0.625rem] font-bold uppercase tracking-wider text-emerald-400 sm:flex"><RefreshCw size="0.75rem"/> 30s</span>
                        <LanguageSwitcher/>
                        <Link to={`/dashboard?station=${encodeURIComponent(selectedStation?.station ?? 'ALL')}`} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-brand-border bg-brand-bg px-3 text-[0.625rem] font-bold text-brand-text-muted hover:border-brand-accent/50 hover:text-brand-text">
                            <BarChart3 size="0.875rem"/><span className="hidden sm:inline">{t('nav_public_dashboard')}</span>
                        </Link>
                        <Link to="/" className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-brand-border bg-brand-bg px-3 text-[0.625rem] font-bold text-brand-text-muted hover:border-brand-accent/50 hover:text-brand-text">
                            <LogIn size="0.875rem"/><span className="hidden sm:inline">{t('dashboard_staff_panel')}</span>
                        </Link>
                    </div>
                </div>
            </header>
            <main className={`mx-auto w-full px-5 sm:px-8 ${stationRowMode ? 'lg:min-h-0 lg:flex-1 lg:py-[clamp(0.55rem,1.45vh,1.5rem)]' : 'py-6'}`}>
        <div className={`${stationRowMode ? 'lg:h-full lg:min-h-0' : 'space-y-6'} animate-in fade-in slide-in-from-bottom-4 duration-300`} id="live-monitor-container">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="bg-brand-surface p-5 rounded-xl border border-brand-border flex flex-col justify-between relative overflow-hidden group hover:border-brand-accent/40 transition-colors">
                    <div className="flex items-center justify-between">
                        <span className="text-[0.625rem] font-bold uppercase tracking-wider text-brand-text-muted">
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[0.5625rem] font-bold uppercase text-emerald-400">LIVE</span>
                        </div>
                    </div>
                ...
                <div className="bg-brand-surface p-5 rounded-xl border border-brand-border flex flex-col justify-between hover:border-brand-accent/40 transition-colors">
                    <div className="flex items-center justify-between">
                        ...
                        <Package size="1rem" className="text-brand-text-muted"/>
                    </div>
                    <span className="text-4xl font-extrabold text-brand-text mt-3">
                        {data.fleetSummary.total}
                    </span>
                    <span className="text-[0.625rem] font-semibold text-brand-text-muted mt-2">
                ...
                <div className="bg-brand-surface p-5 rounded-xl border border-brand-border flex flex-col justify-between hover:border-brand-accent/40 transition-colors">
                    <div className="flex items-center justify-between">
                        ...
                        <CheckCircle2 size="1rem" className="text-emerald-400"/>
                    </div>
                    <span className="text-4xl font-extrabold text-emerald-400 mt-3">
                        {data.fleetSummary.active}
                    </span>
                    <span className="text-[0.625rem] font-semibold text-emerald-400/90 mt-2">
                ...
                <div className="bg-brand-surface p-5 rounded-xl border border-brand-border flex flex-col justify-between hover:border-brand-accent/40 transition-colors">
                    <div className="flex items-center justify-between">
                        ...
                        <Wrench size="1rem" className="text-amber-400"/>
                    </div>
                    <div className="flex items-baseline gap-3 mt-3">
                        ...
                        <div className="flex gap-2 text-[0.625rem] font-mono">
                            ...
                            {data.fleetSummary.blocked > 0 && (
                                <span className="text-red-400 font-bold">
                                    {data.fleetSummary.blocked} <ShieldAlert size="0.625rem" className="inline"/>
                                </span>
                            )}
                        </div>
                    </div>
                    <span className="text-[0.625rem] text-brand-text-muted mt-2">
            ...
            {visibleProjects.length === 0 ? (
                <div className="bg-brand-surface rounded-xl border border-brand-border p-12 text-center">
                    <Package size="2rem" className="mx-auto text-brand-text-muted/40 mb-3"/>
                ...
                <div
                    ...
                    className={stationRowMode
                        ? 'grid gap-3 lg:h-full lg:min-h-0 lg:auto-rows-fr lg:gap-[clamp(0.5rem,1.2vh,0.85rem)]'
                ...
                >
                    {visibleProjects.map((proj: ProjectStats, index) => {
                        ...
                        return (
                            <div
                                key={proj.name}
                                className={`group relative overflow-hidden rounded-xl border border-brand-border bg-brand-surface transition-all duration-300 hover:shadow-lg ${theme.borderAccent} ${stationRowMode ? 'min-h-60 p-4 lg:h-full lg:min-h-60 lg:p-[clamp(0.65rem,1.35vh,1.25rem)]' : 'flex min-h-64 flex-col justify-between p-5'} ${stationRowMode ? 'live-project-card' : ''} ${isNewest ? 'live-project-card-newest' : ''}`}
                                style={!showAll ? {animationDelay: `${Math.min(index * 70, 350)}ms`} : undefined}
                            >
                                {stationRowMode ? (
                                    <div className="grid h-full min-h-0 gap-4 lg:grid-cols-[minmax(0,0.7fr)_minmax(0,0.9fr)_minmax(0,1.8fr)] lg:items-center lg:gap-[clamp(0.75rem,1.4vw,1.75rem)]">
                                        <div className="flex min-h-0 flex-col items-center justify-center border-b border-brand-border/60 pb-4 text-center lg:h-full lg:border-r lg:border-b-0 lg:pr-[clamp(0.75rem,1.4vw,1.75rem)] lg:pb-0">
                                            ...
                                            <h4 className="live-project-name mt-[clamp(0.3rem,0.7vh,0.5rem)] w-full truncate text-[clamp(1.75rem,4vh,3rem)] font-black leading-none text-brand-text transition-colors group-hover:text-brand-accent">{proj.name}</h4>
                                        ...
                                        <div className="flex min-h-0 min-w-0 items-center gap-[clamp(0.65rem,1vw,1rem)] rounded-xl border border-brand-accent/20 bg-brand-bg/60 px-[clamp(0.75rem,1.2vw,1.25rem)] py-[clamp(0.55rem,1.1vh,1.25rem)]">
                                            <div className="grid size-[clamp(2.5rem,5vh,3.5rem)] shrink-0 place-items-center rounded-xl border border-brand-accent/25 bg-brand-accent/10 text-brand-accent">
                                                <ScanBarcode size="1.75rem"/>
                                            </div>
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[0.625rem] font-black uppercase tracking-[0.15em] text-brand-accent">{t('dashboard_last_scanned_pallet')}</span>
                                                    {isNewest && <span className="rounded bg-emerald-400/10 px-1.5 py-0.5 text-[0.5rem] font-black uppercase tracking-wider text-emerald-400">{t('dashboard_newest')}</span>}
                                                </div>
                                        ...
                                        <div className="min-w-0">
                                            <div className="live-metrics grid h-[clamp(6.25rem,14vh,8.75rem)] grid-cols-4 gap-2 sm:gap-[clamp(0.5rem,0.8vw,0.75rem)]">
                                                {[
                                                    {label: t('status_active'), value: ready, color: 'text-emerald-400', panel: 'border-emerald-500/20 bg-emerald-500/[0.07]', icon: <CheckCircle2 size="1.5rem"/>},
                                                    {label: t('status_washing_required'), value: proj.washing, color: 'text-cyan-400', panel: 'border-cyan-500/20 bg-cyan-500/[0.07]', icon: <RefreshCw size="1.5rem"/>},
                                                    {label: t('damaged_status'), value: proj.damaged, color: 'text-rose-400', panel: 'border-rose-500/20 bg-rose-500/[0.07]', icon: <Wrench size="1.5rem"/>},
                                                    {label: t('status_blocked'), value: proj.blocked, color: 'text-orange-400', panel: 'border-orange-500/20 bg-orange-500/[0.07]', icon: <ShieldAlert size="1.5rem"/>},
                                                ].map((metric) => (
                                                    <div key={metric.label} className={`grid min-h-0 min-w-0 grid-rows-[minmax(2.25rem,1fr)_clamp(1.25rem,2.6vh,2rem)_clamp(1.75rem,3.8vh,2.75rem)] place-items-center overflow-hidden rounded-xl border px-2 py-[clamp(0.3rem,0.7vh,0.65rem)] text-center sm:px-3 ${metric.panel}`}>
                                                        <p className={`live-metric-value flex min-h-0 items-center justify-center font-mono text-[clamp(2.1rem,4.8vh,3.75rem)] font-black leading-none ${metric.color}`}>{metric.value}</p>
                                    ...
                                    <>
                                        <div>
                                            <div className="mb-4 flex items-start justify-between gap-2">
                                                <div className="min-w-0 space-y-1">
                                                    <span className="text-[0.625rem] font-semibold uppercase tracking-wider text-brand-text-muted">
                                                        {t('project')}
                                                    </span>
                                                ...
                                                <div className="mb-4 rounded-lg border border-brand-accent/20 bg-brand-bg/60 p-3">
                                                    <div className="flex items-center justify-between gap-3">
                                                        <div className="flex min-w-0 items-center gap-2.5">
                                                            <ScanBarcode size="1.0625rem" className="shrink-0 text-brand-accent"/>
                                                            <div className="min-w-0">
                                                                <p className="truncate font-mono text-base font-black tracking-[0.06em] text-brand-text">{stationEntry.pallet_id}</p>
                                                                <p className="mt-0.5 truncate text-[0.625rem] font-semibold text-brand-text-muted">{stationEntry.model}</p>
                                                            </div>
                                                        </div>
                                                        <div className="shrink-0 text-right">
                                                            {isNewest && <p className="text-[0.5rem] font-black uppercase tracking-[0.14em] text-emerald-400">{t('dashboard_newest')}</p>}
                                                            <p className="mt-0.5 text-[0.625rem] font-bold text-brand-text-muted">{formatLastScanTime(stationEntry.updated_at, language)}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
                                        ...
                                        <div>
                                            <p className="text-[0.625rem] uppercase font-semibold text-brand-text-muted">
                                                {t('ready_total')}
                                            </p>
                                        ...
                                        <div className="space-y-1.5 pt-1">
                                            ...
                                            {total === 0 && (
                                                <p className="text-center text-[0.625rem] italic text-brand-text-muted">
