...
try {
    ...
    await page.route('**/*', async route => {
        ...
        if (projectMatch) {
            ...
            if (req.method() === 'PUT') { operations.push({action: 'update', kind: 'project', id, name: body.name}); const r = projects.find(x => x.id === id); if (r) r.name = body.name; }
            else if (req.method() === 'DELETE') { operations.push({action: 'delete', kind: 'project', id}); const idx = projects.findIndex(x => x.id === id); if (idx >= 0) projects.splice(idx, 1); }
        } else if (modelMatch) {
            const id = Number(modelMatch[1]);
            if (req.method() === 'PUT') { operations.push({action: 'update', kind: 'model', id, name: body.name}); const r = models.find(x => x.id === id); if (r) r.name = body.name; }
            else if (req.method() === 'DELETE') { operations.push({action: 'delete', kind: 'model', id}); const idx = models.findIndex(x => x.id === id); if (idx >= 0) models.splice(idx, 1); }
        }
    ...
    await page.getByRole('button',{name:/Edytuj nazwę: PROJECT-A MODEL-B/}).click();await dialog.locator('input').fill('MODEL-C');await submit();assert.deepEqual(operations[0],{action:'update',kind:'model',id:2,name:'MODEL-C'});
    ...
    await page.getByRole('button',{name:/Usuń wpis: PROJECT-A MODEL-C/}).click();await dialog.getByRole('button',{name:'Usuń wpis',exact:true}).click();await dialog.waitFor({state:'hidden'});assert.deepEqual(operations[1],{action:'delete',kind:'model',id:2});
    ...
    const geometry = await table.evaluate(element => ({
        ...
        track: parseFloat(getComputedStyle(element).getPropertyValue('--admin-header-height')),
